...
import {
  FaBell, FaTicketAlt, FaCalendarAlt, FaCheckCircle,
  FaCreditCard, FaTimes, FaCheck,
} from 'react-icons/fa';
...
export interface Notification {
  id: number;
  type: 'purchase' | 'upcoming_event' | 'system';
  title: string;
  body: string;
  read: boolean;
  createdAt: string;
...
const TYPE_CONFIG = {
  purchase:       { icon: FaCreditCard,  bg: '#f0fdf4', color: '#10b981' },
  upcoming_event: { icon: FaCalendarAlt, bg: '#eff6ff', color: '#3b82f6' },
...
export default function NotificationDropdown({ lightBg }: Props) {
  ...
  // Carrega ao abrir o dropdown e a cada 60s
  useEffect(() => {
    if (!token) return;
    fetchNotifications();
        ...
        <div style={{
          ...
          <div style={{ maxHeight: '380px', overflowY: 'auto' }}>
            {loading && notifications.length === 0 ? (
              <div style={{ padding: '32px 20px', textAlign: 'center', color: '#9ca3af', fontSize: '0.85rem' }}>
                Carregando...
              </div>
            ) : !token ? (
              <div style={{ padding: '32px 20px', textAlign: 'center' }}>
                <FaBell style={{ fontSize: '2rem', color: '#d1d5db', marginBottom: '8px' }} />
                <p style={{ color: '#9ca3af', fontSize: '0.875rem' }}>Faça login para ver notificações</p>
              </div>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                        <p style={{ fontWeight: n.read ? 500 : 700, color: '#111827', fontSize: '0.83rem', lineHeight: 1.3 }}>
                          {n.title}
                        </p>
                        <button
                      ...
                      <p style={{ color: '#94a3b8', fontSize: '0.72rem', marginTop: '4px' }}>{timeAgo(n.createdAt)}</p>
                    </div>
          ...
          {notifications.length > 0 && (
            <div style={{ padding: '12px 20px', borderTop: '1px solid #f1f5f9', textAlign: 'center' }}>
              <button
                ...
                style={{ fontSize: '0.8rem', color: '#00C2A8', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}
              >
