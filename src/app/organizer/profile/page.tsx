...
import { FaArrowLeft, FaSave, FaCheckCircle, FaBuilding, FaUniversity, FaUser } from 'react-icons/fa';
...
export default function OrganizerProfilePage() {
        ...
        if (userRes.ok) {
          ...
          if (u.bankInfo) {
            try {
              ...
              // bankInfo não é JSON válido, ignorar
            }
          }
        }
  ...
  const setOrgField = (k: string, v: string) =>
    setOrgForm((prev) => ({ ...prev, [k]: v }));

  const setBankField = (k: string, v: string) =>
    setBankForm((prev) => ({ ...prev, [k]: v }));
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      // 1. Salvar dados bancários no usuário
      const bankInfoJson = JSON.stringify(bankForm);
      const userRes = await fetch(`${API_URL}/users/${userId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ bankInfo: bankInfoJson }),
      });
      if (!userRes.ok) {
      ...
      // 2. Salvar perfil do organizador (se existe)
      if (organizerId) {
        const orgRes = await fetch(`${API_URL}/organizers/${organizerId}`, {
          ...
          body: JSON.stringify({
            ...
            zipcode: orgForm.zipcode || undefined,
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" className="input-form bg-gray-50" value={orgForm.cnpj} readOnly
